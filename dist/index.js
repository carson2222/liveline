...
function drawGrid(ctx, layout, palette, formatValue, state, dt) {
  ...
  const coarse = pickInterval(valRange, pxPerUnit, 60, state.interval);
...
function drawReferenceLine(ctx, layout, palette, ref, rightAligned) {
...
function Liveline({
  ...
  useLivelineEngine(canvasRef, containerRef, {
    ...
    referenceLine,
    referenceLines: referenceLines ?? (referenceLine ? [referenceLine] : []),
    formatValue,
    formatTime,
    padding: pad,
    onHover,
